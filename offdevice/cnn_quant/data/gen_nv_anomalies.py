"""Synthesize anomalous 4 KB NV regions against spec v2.

    synthesize(nv, spec_dict, rng, ...) is a parameterized generator.
    ...
    Run with:
        python -m offdevice.cnn_quant.data.gen_nv_anomalies --per-type 30
"""
...
from offdevice.nv.parse import journal_chain, parse_region, slice_nv
...
def synthesize(nv: bytes, spec_dict: dict, rng: random.Random,
    ...
    (for the battery manifest). code_src supplies realistic code-content bytes
    for blobs (e.g. the base capture's own firmware region).
    """
    ...
    if atype == "out_of_range":
        ...
        # byte pattern 127 real reboots left in the clean set. Channel values
        # stay smooth (environments don't reboot). No spec invariant can
        # separate this from a real reset without cross-snapshot state.
        if n < 12:
            return None
        k0 = p.get("slot")
        if k0 is None:
        ...
        if n_chain == 0:
            return None
        hdr = view.pages[cur_off // spec.PAGE_SIZE].header
        ...
        if mode == "reserved0":
            ...
            if mode == "op_backwards":
                # what the real steady1_run003 anomaly is: a chain entry whose
                # op_count steps BACKWARDS (replayed older settings state)
        ...
        if mode == "full_smash":
            ...
            # the 1-byte flip finding 3 calls out: header fails validation, the
            # page's records go silently invisible to a chronological parse
...
# =============================================================================
# CLI -- quick per-type battery for dataset work. The exam's graded battery is
# generated by offdevice/exam/gen_battery.py (exam seed, committed manifest);
# this CLI is for ad-hoc training/eval sets. The retired legacy generator
# (make_anomaly, seed-99 set) lives in git history at the exam freeze commit --
# frozen.json pins that file version by sha256.
# =============================================================================

# fill-state bins by record count (Sean's states: empty..steady). Rare bins --
# near-empty captures -- were starved by uniform sampling, so the CNN never
# learned tampers on them (the top1/fill1 real misses). Bin then sample per-bin.
FILL_BINS = ((0, 20), (20, 100), (100, 200), (200, 245))


def _stratify(bases):
    """Group bases by record-count bin so each fill state is sampled evenly."""
    groups = {b: [] for b in FILL_BINS}
    for f in bases:
        n = _n_records(slice_nv(f.read_bytes()))
        for lo, hi in FILL_BINS:
            if lo <= n < hi:
                groups[(lo, hi)].append(f)
                break
    return [g for g in groups.values() if g]   # drop empty bins


def main():
    ap = argparse.ArgumentParser()
    repo = Path(__file__).resolve().parents[3]
    ap.add_argument("--benign-dir", type=Path, default=repo / "offdevice" / "data" / "captures")
    ap.add_argument("--out-dir", type=Path,
                    default=Path(__file__).resolve().parent / "synthetic_anomalies")
    ap.add_argument("--per-type", type=int, default=30)
    ap.add_argument("--seed", type=int, default=99)
    ap.add_argument("--uniform", action="store_true",
                    help="uniform base sampling (default: stratified by fill state)")
    args = ap.parse_args()

    bases = sorted(args.benign_dir.glob("benign__*.bin"))
    if not bases:
        raise SystemExit(f"no benign bases in {args.benign_dir}")
    strata = None if args.uniform else _stratify(bases)
    if strata:
        print(f"stratified base sampling: {[len(g) for g in strata]} captures per fill bin")
    args.out_dir.mkdir(parents=True, exist_ok=True)
    manifest = (args.out_dir / "manifest.jsonl").open("w")
    rng = random.Random(args.seed)

    n_written = 0
    for atype in SYNTH_TYPES:
        made = 0
        tries = 0
        while made < args.per_type and tries < args.per_type * 5:
            tries += 1
            # stratified: pick a fill bin round-robin, then a base within it
            if strata:
                base = rng.choice(strata[made % len(strata)])
            else:
                base = rng.choice(bases)
            nv = slice_nv(base.read_bytes())
            out = synthesize(nv, {"type": atype}, rng)
            if out is None:
                continue
            data, params = out
            name = f"synth__{atype}__{made:03d}.bin"
            (args.out_dir / name).write_bytes(data)
            manifest.write(json.dumps({
                "file": name, "label": "anomalous", "anomaly_type": atype,
                "base": base.name, "md5": hashlib.md5(data).hexdigest(),
                "params": params,
            }) + "\n")
            made += 1
            n_written += 1
        print(f"  {atype:22s} {made}")
    manifest.close()
    print(f"\nwrote {n_written} synthetic anomalies to {args.out_dir}")
